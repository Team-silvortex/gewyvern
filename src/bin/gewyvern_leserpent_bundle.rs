    ...
    fn parse(args: impl Iterator<Item = String>) -> Result<Self, String> {
        ...
        while let Some(argument) = args.next() {
            let value = match argument.as_str() {
                "--publish-dir" | "--output" | "--icon" | "--version" => args
                    .next()
...
fn create_bundle(options: &Options) -> Result<(), String> {
    ...
    let files = publish_files(&options.publish_dir)?;
    if !files
        .iter()
...
fn verify_bundle(bundle: &Path, version: &str) -> Result<(), String> {
    ...
    #[cfg(unix)]
    {
        ...
        if fs::metadata(&executable)
            ...
            == 0
        {
            return Err("bundled executable has no execute permission".to_string());
        }
    }
    require_file(
...
fn usage() -> &'static str {
    "usage: gewyvern_leserpent_bundle --publish-dir DIR --output Leserpent.app [--icon FILE] [--version X.Y.Z]"
}
