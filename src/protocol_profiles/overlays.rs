...
pub(super) fn overlays_for_surface(protocol: &str, entry: &str) -> Vec<ProtocolOverlaySummary> {
    match (protocol, entry) {
        ...
        ("http3", "close") => vec![ProtocolOverlaySummary {
            key: "http3-close".into(),
            label: "HTTP/3 Termination Over QUIC".into(),
            kind: "quic_failure_overlay".into(),
            operator_hint: "Treat this as an HTTP/3 application path collapsing through QUIC connection close; inspect the QUIC close shelf when payload meaning becomes secondary to who terminated first.".into(),
            aliases: vec!["http3-close".into(), "h3-close".into()],
            companion_protocol: Some("quic".into()),
            companion_entry: Some("close".into()),
        }],
        ("http3", "server-close") => vec![ProtocolOverlaySummary {
            key: "http3-server-close".into(),
            label: "HTTP/3 Server Termination Over QUIC".into(),
            kind: "quic_failure_overlay".into(),
            operator_hint: "Treat this as a server-side HTTP/3 response path that actively emitted QUIC connection close after response work had already started; compare against the generic QUIC close shelf when transport cause matters more than application role.".into(),
            aliases: vec!["http3-server-close".into(), "h3-server-close".into()],
            companion_protocol: Some("quic".into()),
            companion_entry: Some("local-close".into()),
        }],
        ("hy2", "auth" | "udp" | "tcp") => vec![ProtocolOverlaySummary {
            key: "hy2".into(),
            label: "Hysteria2 Over QUIC".into(),
            kind: "quic_application_overlay".into(),
            operator_hint: "Treat this as Hysteria2 semantics layered on top of QUIC setup; if auth or relay meaning is missing, inspect QUIC initial and crypto progress first.".into(),
            aliases: vec!["hy2".into(), "hysteria2".into()],
            companion_protocol: Some("quic".into()),
            companion_entry: Some("initial".into()),
        }],
        ("hy2", "close") => vec![ProtocolOverlaySummary {
            key: "hy2-close".into(),
            label: "Hysteria2 Termination Over QUIC".into(),
            kind: "quic_failure_overlay".into(),
            operator_hint: "Treat this as an authenticated Hysteria2 path collapsing through QUIC connection close; inspect the QUIC close shelf when relay or auth semantics stop advancing.".into(),
            aliases: vec!["hy2-close".into(), "hysteria2-close".into()],
            companion_protocol: Some("quic".into()),
            companion_entry: Some("close".into()),
        }],
        ("hy2", "tcp-close") => vec![ProtocolOverlaySummary {
            key: "hy2-tcp-close".into(),
            label: "Hysteria2 TCP Relay Termination Over QUIC".into(),
            kind: "quic_failure_overlay".into(),
            operator_hint: "Treat this as a TCP-shaped Hysteria2 relay that already authenticated and exchanged relay payloads before the peer emitted QUIC connection close; compare against generic HY2 close when relay role is still ambiguous.".into(),
            aliases: vec!["hy2-tcp-close".into(), "hysteria2-tcp-close".into()],
            companion_protocol: Some("quic".into()),
            companion_entry: Some("close".into()),
        }],
        ("hy2", "udp-close") => vec![ProtocolOverlaySummary {
            key: "hy2-udp-close".into(),
            label: "Hysteria2 UDP Relay Termination Over QUIC".into(),
            kind: "quic_failure_overlay".into(),
            operator_hint: "Treat this as a UDP-shaped Hysteria2 relay that already authenticated and exchanged relay datagrams before the peer emitted QUIC connection close; compare against generic HY2 close when relay role is still ambiguous.".into(),
            aliases: vec!["hy2-udp-close".into(), "hysteria2-udp-close".into()],
            companion_protocol: Some("quic".into()),
            companion_entry: Some("close".into()),
        }],
        ("quic", "initial" | "retry" | "crypto" | "close" | "local-close" | "stream" | "bidi") => vec![ProtocolOverlaySummary {
            key: "http3".into(),
            label: "HTTP/3 Over QUIC".into(),
            kind: "quic_application_overlay".into(),
