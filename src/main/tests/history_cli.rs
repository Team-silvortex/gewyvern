...
fn list_history_json_renders_empty_index_when_history_is_missing() {
    ...
    assert!(rendered.contains("\"minor_line\":\"v0.19.x\""));
...
fn list_history_text_reports_existing_snapshots() {
    ...
    assert!(rendered.contains("minor line: v0.19.x"));
    ...
    assert!(rendered.contains("- 1003 line=v0.19.x"));
    assert!(rendered.contains("protocol_catalog="));
    assert!(rendered.contains("- 1001 line=v0.19.x"));
