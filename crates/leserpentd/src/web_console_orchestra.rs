...
use leserpent_domain::{
    CAPABILITY_ORCHESTRA_WRITE, CapabilitySet, CommandId, Principal, RuntimeId, RuntimeProjection,
};
use leserpent_protocol::compatibility_v1::{LegacyOrchestraEvent, LegacyOrchestraRun};
...
pub(crate) fn reject_session_handoff(body: &[u8]) -> Result<Value, ConsoleWriteError> {
    let request: SessionHandoffRequest = decode_request(body)?;
    required_text(Some(&request.pipeline_kind), 128)?;
    required_text(Some(&request.requested_by), 80)?;
    Err(ConsoleWriteError::new(
        ConsoleWriteStatus::Conflict,
        "orchestra_session_authority_unavailable",
        "generic session handoff has no Rust domain authority",
    ))
}
...
mod tests {
    ...
    #[test]
    fn compatibility_requests_are_strict_and_session_handoff_is_honest() {
        ...
        let error =
            reject_session_handoff(br#"{"pipelineKind":"diagnostic","requestedBy":"operator"}"#)
                .unwrap_err();
        assert_eq!(error.status, ConsoleWriteStatus::Conflict);
        assert_eq!(error.code, "orchestra_session_authority_unavailable");
