...
- can we still ship `1.14.x` with confidence?
...
Treat `1.14.x` as release-ready only when all of the following stay true:
...
names; `1.14.x` identifies the active minor line and is never itself a package
version.
...
For the active `1.14.x` line, a good practical ship read is:
