...
- can we still ship `1.7.x` with confidence?
...
Treat `1.7.x` as release-ready only when all of the following stay true:
...
names; `1.7.x` identifies the active minor line and is never itself a package
version.
...
For the active `1.7.x` line, a good practical ship read is:
