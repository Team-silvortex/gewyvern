...
- current release judgment for the active `1.20.x` line
...
Keep the current `1.20.x` field-validation line intentionally narrow.
...
It exists so the active `1.20.x` line has a concrete "keep validating for
real" shelf rather
