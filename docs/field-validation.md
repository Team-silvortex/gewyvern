...
- current release judgment for the active `2.0.x` line
...
Keep the current `2.0.x` field-validation line intentionally narrow.
...
It exists so the active `2.0.x` line has a concrete "keep validating for
real" shelf rather
