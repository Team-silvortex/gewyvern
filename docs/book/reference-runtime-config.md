...
Before selecting the file, active `2.0.x` startup prepares the standard runtime
...
This policy surface is intentionally conservative. In the active `2.0.x`
line it highlights:
...
The current `2.0.x` startup behavior now pairs this config contract with a
