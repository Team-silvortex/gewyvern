...
Before selecting the file, active `0.20.x` startup prepares the standard runtime
...
This policy surface is intentionally conservative. In the active `0.20.x`
line it highlights:
...
The current `0.20.x` startup behavior now pairs this config contract with a
