...
Before selecting the file, active `1.12.x` startup prepares the standard runtime
...
This policy surface is intentionally conservative. In the active `1.12.x`
line it highlights:
...
The current `1.12.x` startup behavior now pairs this config contract with a
