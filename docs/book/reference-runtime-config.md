...
Before selecting the file, active `1.0.0` startup prepares the standard runtime
...
This policy surface is intentionally conservative. In the active `1.0.0`
line it highlights:
...
The current `1.0.0` startup behavior now pairs this config contract with a
