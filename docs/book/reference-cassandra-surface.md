...
Protocol aliases: `cql`, `cql-startup`, `cql_startup`, `cassandra-startup`, `cassandra_startup`, `cassandra-query`, `cassandra_query`, `cql-query`, `cql_query`, `cql-result`, `cql_result`, `cassandra-result`, `cassandra_result`, `cql-error`, `cql_error`, `cassandra-error`, `cassandra_error`
...
Start with `query` for normal CQL request debugging. Use `startup` when a client never gets a stable session. Use `result` to check response directionality, and `error` when Cassandra explicitly rejects or fails a request.
