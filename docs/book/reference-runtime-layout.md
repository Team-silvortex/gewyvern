...
The standard config file name for the active `2.1.x` line is:
...
The current `2.1.x` expectation is:
...
The current `2.1.x` history retention rule is intentionally simple:
...
In the current `2.1.x` line those JSON artifacts may include:
