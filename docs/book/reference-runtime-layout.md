...
The standard config file name for the active `1.7.x` line is:
...
The current `1.7.x` expectation is:
...
The current `1.7.x` history retention rule is intentionally simple:
...
In the current `1.7.x` line those JSON artifacts may include:
