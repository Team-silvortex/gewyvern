# Project Documentation Module

Owns contributor workflow, source ownership, stack integration, packaging
maintenance, and historical context.

## Start

1. [Development guide](../development.md)
2. [Module boundaries](../module-boundaries.md)
3. [Monorepo stack](../monorepo-stack.md)
4. [Project status tensor](../project-status-system.md)
5. [Live status entry](../../STATUS.md)

## Maintain

- [Architecture coordination](../architecture-coordination.md)
- [Sidecar collaboration](../sidecar-collaboration.md)
- [External-engine contract](../external-engine-contract.md)
- [Packaging](../packaging.md)
- [Documentation system](../documentation-system.md)
- [Release checklist](../release-checklist.md)
- [Leserpent 2.0 architecture](../leserpent-2-architecture.md)
- [Leserpent 2.0 roadmap](../leserpent-2-roadmap.md)
- [Project status protocol](../project-status-system.md)

## History

- [History index](../history/index.md)
- [1.0.0 release notes](../history/v1.0.0-release-notes.md)
- [1.0.0 posture](../history/v1.0.0.md)
- [Roadmap](../../ROADMAP.md)

History records previous decisions; current contracts always win when the two
disagree.
