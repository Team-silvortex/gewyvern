# Operations Documentation Module

Owns commands, deployment, runtime validation, security checks, performance,
and release gates.

## Start

1. [First run](../book/tutorial-first-run.md)
2. [CLI recipes](../cli-recipes.md)
3. [Native validation commands](../script-entrypoints.md)

## Operate

- [Service behavior](../service-behavior.md)
- [Ingest modes](../ingest-modes.md)
- [Headless Linux](../headless-linux.md)
- [Remote Docker execution](../remote-docker.md)
- [Runtime configuration](../runtime-config-contract.md)
- [Security checklist](../book/how-to-security-checklist.md)
- [Security posture](../security-posture.md)

## Validate And Ship

- [Runtime validation](../book/how-to-validate-runtime-surface.md)
- [Fault injection](../book/how-to-fault-inject-runtime-resilience.md)
- [Field validation](../field-validation.md)
- [Field findings](../field-findings.md)
- [Performance baselines](../performance-baselines.md)
- [Packaging](../packaging.md)
- [Release checklist](../release-checklist.md)

Architecture and contract rationale belong to the
[runtime module](runtime.md).
