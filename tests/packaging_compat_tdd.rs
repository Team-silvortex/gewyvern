...
fn package_layout_writes_compat_manifest() {
    ...
    assert!(build_script.contains("RELEASE_LINE=\"${GEWY_RELEASE_LINE:-v1.12.2}\""));
    ...
    assert!(build_script.contains(".package-rpm.XXXXXX"));
...
fn install_smoke_validates_packaged_compat_artifacts() {
    ...
    assert!(harness.contains("RELEASE_LINE=\"${GEWY_RELEASE_LINE:-v1.12.2}\""));
