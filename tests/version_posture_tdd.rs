...
fn etragon_inherits_the_workspace_version() {
    ...
    assert_eq!(workspace_version, "1.15.0");
...
fn docs_describe_the_active_minor_line_without_component_versions() {
    ...
    let mut version_parts = workspace_version.split('.');
    let active_line = format!(
        "{}.{}.x",
        version_parts.next().unwrap(),
        version_parts.next().unwrap()
    );
    assert!(readme.starts_with(&format!("# gewyvern v{active_line}\n")));
    assert!(readme.contains(&format!("project version: `{active_line}`")));
    assert!(readme.contains(&format!("current release line: `v{active_line}`")));
...
fn release_checklist_uses_version_template_for_package_artifacts() {
    ...
    assert!(!checklist.contains("gewyvern_1.15.x-1_"));
    assert!(!checklist.contains("gewyvern-1.15.x-1."));
