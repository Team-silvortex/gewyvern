...
fn linux_publish_builds_and_installs_the_rust_compatibility_bridge() {
    ...
    let project = std::fs::read_to_string(
        root.join("apps/leserpent/src/Leserpent/Leserpent.csproj"),
    )
    .unwrap();
    ...
    assert!(installer.contains(
        "for required in Leserpent leserpent-compat-bridge libe_sqlite3.so"
    ));
    ...
    assert!(environment.contains(
        "LESERPENT_RUST_BRIDGE_BIN=/opt/leserpent/current/leserpent-compat-bridge"
    ));
