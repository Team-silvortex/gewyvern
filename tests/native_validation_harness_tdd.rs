...
fn leserpent_native_aot_proof_is_native_and_fail_closed() {
    ...
    assert!(harness.contains("renderer-presentation-conformance-v1.json"));
    assert!(harness.contains("--verify-focus-retention"));
    assert!(harness.contains("native_presentation_fixture"));
    assert!(harness.contains("native_action_activation"));
    assert!(harness.contains("native_click_exactly_once=true"));
    assert!(harness.contains("unavailable_action_rejected=true"));
    assert!(harness.contains("hidden_action_rejected=true"));
    assert!(harness.contains("non_action_rejected=true"));
    assert!(harness.contains("missing_action_rejected=true"));
    assert!(harness.contains("reopen_window=true"));
    assert!(harness.contains("reclose_window=true"));
    assert!(harness.contains("window_lifecycle_idempotent=true"));
    assert!(harness.contains("window_reopen_fresh_native_window=true"));
...
fn leserpent_accessibility_proof_audits_real_controls_and_contrast() {
    ...
    assert!(harness.contains("schema_version\": 2"));
    ...
    assert!(harness.contains("six_product_function_chain_probes"));
    assert!(harness.contains("strict_product_probe_success_markers"));
    assert!(harness.contains("--verify-hub-topology"));
    assert!(harness.contains("--verify-remote-shell-controls"));
    assert!(harness.contains("--verify-bootstrap-controls"));
    assert!(harness.contains("--verify-provisioning-controls"));
    assert!(harness.contains("--verify-retirement-controls"));
    assert!(harness.contains("--verify-daemon-retirement-controls"));
    assert!(harness.contains("late_completion_close_fence=true"));
    assert!(harness.contains("polling_restart_after_close=false"));
    assert!(harness.contains("settled_lifetime_disposal=true"));
    assert!(harness.contains("product_function_chains"));
    assert!(harness.contains("probe-{}.log"));
