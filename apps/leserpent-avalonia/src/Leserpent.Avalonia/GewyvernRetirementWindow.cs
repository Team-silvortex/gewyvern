...
internal sealed class GewyvernRetirementWindow : Window
{
    ...
    {
        Content = "I confirm service removal and runtime unregistration on this host",
        Foreground = LeserpentTheme.Body,
    };
    private readonly Button submitButton = DestructiveButton("Retire gewyvern");
    ...
    private readonly TextBlock status = new()
    {
        ...
        Text = "Choose the daemon authority that owns this registered runtime.",
    };
        ...
        RetirementHubOperations operations)
    {
        ...
        Audit(authority, "retirement-authority", "Daemon authority owning the gewyvern runtime");
        Audit(retirementId, "retirement-id", "Stable gewyvern retirement operation ID");
        Audit(provisioningId, "retirement-provisioning-id", "Original provisioning authority ID");
        Audit(runtimeId, "retirement-runtime-id", "Registered runtime ID to retire");
        Audit(host, "retirement-host", "Target host for gewyvern retirement");
        Audit(port, "retirement-port", "Target SSH port");
        Audit(credentialHandle, "retirement-credential-handle", "Opaque SSH credential handle");
        Audit(confirmation, "retirement-confirm", "Confirm service removal and unregistration");
        Audit(submitButton, "retirement-submit", "Retire and unregister gewyvern runtime");
        Audit(refreshButton, "retirement-refresh", "Refresh the same retirement attempt");
        Audit(close, "retirement-close", "Close gewyvern retirement window");
        Audit(status, "retirement-status", "Gewyvern retirement status");
        Audit(phase, "retirement-phase", "Gewyvern retirement phase");
                ...
                Children =
                {
                    new StackPanel
                    {
                        ...
                        Children =
                        {
                            ...
                            new TextBlock
                            {
                                Text = "Stop, remove, then unregister",
                                Foreground = LeserpentTheme.Primary,
                                FontSize = 27,
                                FontWeight = FontWeight.Bold,
                            },
                            new TextBlock
                            {
                                Text = "The selected leserpentd must prove that the bound gewyvern service was retired before it atomically unregisters the runtime. Failure preserves the registration for recovery.",
                                Foreground = LeserpentTheme.Muted,
                                TextWrapping = TextWrapping.Wrap,
                            },
                        },
                    ...
                    new Border
                    {
                        ...
                        Child = new StackPanel
                        {
                            ...
                            Children =
                            {
                                ...
                                Field("Original provisioning ID", provisioningId),
    ...
    private async Task SubmitAsync()
    {
        ...
        try
        {
            ...
            {
                throw new ArgumentException("Confirm gewyvern retirement before submitting.");
            }
    ...
    private void RenderSnapshot(RemoteRetirementSnapshot state)
    {
        ...
        {
            "planned" => "Retirement is durably queued. Observation reuses this exact identity and does not submit a second removal.",
            "retiring_service" => "The daemon authority is stopping and removing the bound gewyvern service.",
            "service_retired" => "The service retirement proof is committed; atomic runtime unregistration is pending.",
            "runtime_unregistered" => $"Runtime {Safe(state.RuntimeId)} was safely unregistered after service retirement.",
            "failed" => $"Retirement failed with bounded fault {Safe(state.FaultCode)}. The runtime remains registered for inspection and recovery; use a new retirement ID for a corrected attempt.",
