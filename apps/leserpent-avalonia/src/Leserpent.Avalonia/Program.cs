...
internal static class Program
{
    ...
    public static int Main(string[] args)
    {
        ...
        if (args is ["--verify-registration-client"])
        {
            RemoteRegistrationClient.VerifyContract();
            Console.WriteLine(
                "runtime registration client valid: inspect=true, dry_run=true, explicit_confirmation=true, update_revision_fence=true, secret_free=true, strict_decode=true");
            return 0;
        }
        if (args is ["--verify-orchestra-client"])
        {
            RemoteOrchestraClient.VerifyContract();
            Console.WriteLine(
                "Orchestra client valid: rust_authority=true, authenticated_wire=true, native_plans=true, run=true, queued_cancel=true, retry_lineage=true, history=true, event_history=true, idempotent_cleanup=true, strict_decode=true, identity_fenced=true, bounded=true");
            return 0;
        }
        ...
        if (args is ["--verify-bootstrap-promotion"])
        {
            ...
            Console.WriteLine(
                "bootstrap promotion valid: session_bound_only=true, endpoint_trust_bound=true, rust_secret_handle=true, health_before_persist=true, platform_vault=true, catalog_secret_free=true, conflicting_credential_rejected=true, global_ca_gc=false, unrelated_managed_ca_preserved=true");
            return 0;
        }
        ...
        if (args is ["--verify-desktop-ca-store"])
        {
            ...
            Console.WriteLine(
                "desktop CA store valid: single_pem=true, certificate_authority=true, content_addressed=true, atomic=true, private=true, idempotent=true, profile_migration=true, bootstrap_handle=true, endpoint_bound=true, digest_bound=true, bounded_prune=true, validation_before_prune=true, retained_content_revalidation=true, stale_temp_cleanup=true, unknown_fields=false, trailing_material=false, managed_replacement=false, symlink=false");
            return 0;
        }
        ...
        if (args is ["--verify-desktop-localization"])
        {
            ...
            DesktopLanguagePackStore.VerifyContract();
            DesktopLanguagePackCatalogClient.VerifyContractAsync()
                .GetAwaiter()
                .GetResult();
            Console.WriteLine(
                "desktop localization valid: schema=v1, official_locales=30, builtin_locales=8, complete_builtin_locales=8, builtin_shell_catalogs=8, native_shell_keys=80, builtin_semantic_catalogs=7, semantic_keys=26, builtin_connection_catalogs=7, connection_semantic_keys=33, builtin_bootstrap_catalogs=7, bootstrap_semantic_keys=46, builtin_provisioning_catalogs=7, provisioning_semantic_keys=43, builtin_retirement_catalogs=7, retirement_semantic_keys=45, builtin_daemon_retirement_catalogs=7, daemon_retirement_semantic_keys=37, builtin_startup_recovery_catalogs=7, startup_recovery_semantic_keys=9, builtin_account_catalogs=7, account_semantic_keys=37, builtin_remote_shell_catalogs=7, remote_shell_semantic_keys=56, builtin_remote_operation_catalogs=7, remote_operation_semantic_keys=57, builtin_runtime_workspace_catalogs=7, runtime_workspace_semantic_keys=78, builtin_orchestra_catalogs=7, orchestra_semantic_keys=72, builtin_debugger_catalogs=7, debugger_semantic_keys=33, builtin_registration_catalogs=7, registration_semantic_keys=49, builtin_hub_catalogs=7, hub_semantic_keys=69, builtin_tutorial_catalogs=7, tutorial_semantic_keys=61, builtin_semantic_keys=751, downloadable_locales=22, language_pack_schema=v1, language_pack_catalog_schema=v1, language_pack_core_ui_keys=18, language_pack_official_version=1.2.0, language_pack_official_keys=42, language_pack_sha256=true, language_pack_catalog_locale_binding=true, language_pack_catalog_version_binding=true, language_pack_same_origin=true, language_pack_bearer_sent=false, language_pack_cancellation_fenced=true, language_pack_atomic_install=true, language_pack_official_precommit=true, language_pack_failed_update_preserves_previous=true, language_pack_private_store=true, language_pack_bad_sibling_isolation=true, language_pack_builtin_replacement=false, language_pack_projection=true, system_resolution=true, bcp47_aliases=true, persistent_preference=true, atomic=true, private=true, unknown_fields=false, localized_ui_ir=true, localized_connection_dialog=true, localized_reverse_deployment=true, localized_gewyvern_provisioning=true, localized_gewyvern_retirement=true, localized_daemon_retirement=true, localized_startup_recovery=true, localized_account=true, localized_remote_shell=true, localized_remote_operations=true, localized_runtime_workspace=true, localized_orchestra=true, localized_registration=true, localized_hub=true, localized_tutorial=true, typed_runtime_workspace_status=true, typed_hub_cards=true, english_fallback=true, zh_cn_core=true, zh_cn_tutorial_complete=true, builtin_tutorial_complete=true, rtl_locales=3");
            return 0;
        }
        if (args is [
            "--verify-desktop-language-pack-artifacts",
            var catalogPath,
            var languagePackRoot])
        {
            DesktopLanguagePackCatalogClient.VerifyPublishedArtifacts(
                catalogPath,
                languagePackRoot);
            Console.WriteLine(
                "published desktop language packs valid: catalog_schema=v1, downloadable_locales=22, compatibility_keys=18, official_version=1.2.0, official_keys=42, sha256=true, locale_version_binding=true, official_precommit=true, native_store_roundtrip=true");
            return 0;
        }
        if (args is ["--verify-local-orchestra", var daemonPath])
        {
            ...
            Console.WriteLine(
                "local orchestra valid: rust_daemon=true, loopback_tls=true, ephemeral_token=true, owned_authority=true, runtime_topology_query=true, health_topology_composition=true, authority_bound_live_state=true, credential_free_language_pack_download=true, language_pack_digest_binding=true, language_pack_private_roundtrip=true, language_pack_official_version=1.2.0, language_pack_official_keys=42, private_files=true, minimal_child_environment=true, optional_bootstrap_origin=true, optional_gewyvern_provisioning_origin=true, private_bootstrap_trust=true, package_local_daemon=true, symlink_rejection=true, process_cleanup=true");
            return 0;
        }
        if (args is ["--verify-saved-daemon-language-pack", var savedDaemonPath])
        {
            LocalOrchestraServiceSupervisor.VerifySavedDaemonLanguagePackContract(
                savedDaemonPath);
            Console.WriteLine(
                "saved daemon language pack valid: persisted_catalog=true, saved_connection_source=true, selected_ca_only=true, wrong_ca_rejected=true, bearer_sent=false, admin_token_sent=false, digest_binding=true, private_roundtrip=true, language_pack_official_version=1.2.0, language_pack_official_keys=42, input_immutable=true, process_cleanup=true");
            return 0;
        }
        ...
        if (args is ["--verify-product-access-policy"])
        {
            ProductAccessPolicy.VerifyContract();
            Console.WriteLine(
                "product access policy valid: open_source_core=true, core_license=MIT, core_price=free, current_core_paywall=false, current_core_subscription=false, account_core_gate=false, daemon_credentials_independent=true, subscription_scope=future_hosted_services_only, subscription_requires_account=true, subscription_requires_entitlement=true, login_only_elevation=false");
            return 0;
        }
        if (args is ["--verify-silvortex-account"])
        {
            ...
            Console.WriteLine(
                "Silvortex desktop account valid: reviewed_application=leserpent, reviewed_profile=leserpent_desktop, default_client_id=true, native_client=true, system_browser=true, pkce_s256=true, state=true, nonce=true, response_issuer=true, strict_loopback_http=true, rs256_jwks=true, mfa=true, rotating_vault=true, duplicate_parameters=false, client_secret=false, offline_mode=true, open_source_core=true, core_license=MIT, core_price=free, current_core_paywall=false, account_core_gate=false, login_only_elevation=false, packaged_issuer=true, configuration_binding=true, environment_override=false, cancellation_safe_disposal=true");
            return 0;
        }
        if (args is ["--verify-silvortex-account-proof"])
        {
            ...
            Console.WriteLine(
                "Silvortex desktop proof valid: schema=v2, reviewed_client=true, native_aot_required=true, packaged_macos_config=true, system_browser=true, fresh_session_restore=true, refresh_rotation=true, local_logout=true, binary_binding=true, configuration_binding=true, private_atomic_evidence=true, identity_retained=false, credential_retained=false");
            return 0;
        }
